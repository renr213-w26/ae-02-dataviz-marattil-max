...
ggplot(bechdel, aes(x = budget_2013, y = gross_2013))+
  ...
  labs(
    x = "Budget ($)", 
    y = "Gross ($)", 
    title = "Relationship between Movie Budget and Total Gross for movies released in 2013, with amounts adjusted to inflation"
  )
```
...
ggplot(bechdel, aes(x = budget_2013, y = gross_2013, colour = binary, size = roi)) +
  ...
  labs(
    x = "Budget (in 2013 $)", 
    y = "Gross revenue (in 2013 $)", 
    title = "Gross revenue vs. budget, by  Bechdel Test result and Return on Investments"
  )
```
...
COMMENTS: Due to the categories under `clean_data`, the all data that would usually fall under the pass binary is already seperated in the `ok` variable.
Therefore, by filtering by the `binary` variable, the display becomes crowded.
I can see how it may be helpful in the event that binary options are not seperate visually by colour.
...
ggplot(bechdel, aes(x = budget_2013, y = gross_2013, colour = binary, size = roi, alpha = 0.5 )) +
  ...
  labs(
    x = "Budget (in billion 2013 $)", 
    y = "Gross revenue (in billion 2013 $)", 
    title = "Gross revenue vs. budget, by Bechdel Test result "
  )+ scale_x_continuous(label = label_number(scale = 1/1000000000))+
  scale_y_continuous(label = label_number(scale = 1/1000000000))
```
...
ggplot(bechdel,
  ...
  geom_boxplot()+
  labs(
    title = "Return on Investments vs. Bechdel Test Results",
    x = "Bechdel Test Results",
